...
class Sequence:
    ...
    def __repr__(self) -> str:
        return f"<Sequence id={self.id} length={self.length} max_new={self.max_new_tokens} context={self.context} completions={self.completions} loglikelihoods={self.loglikelihoods}>"
...
class Batch:
    ...
    def packs_to_batch_factory(cls, train, causal, max_seq_len, batch_size):
        def packs_to_batch(packs: List[List[Sequence]]):
...
def report_logits(sampled_tokens: torch.Tensor, loglikelihoods: torch.Tensor, sampled_tokens_loglikelihoods: torch.Tensor, packs: List[List[Sequence]]):
